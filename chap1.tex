...
of the \latin{Bombyx} fibroins are charaterized by the dipeptide units GA, GS,
...
Wood \latin{et al.} have adapted intein-based cleavage host-domains to 

The work by Meyer and Chilkoti pertaining to the fusion of
...
were generated with the intention of benefiting from the coacervation formation
...
encode for \iupac{2-naphthylalanine}.\cite{Kwon2003}
% --------------------------
...
\begin{equation}
    ...
    \Delta G_{U \rightarrow F}=-RT\ln K_{a}=
    \Delta H_{U \rightarrow F}-
    T \Delta S_{U \rightarrow F}
\end{equation}
...
\begin{equation}
    ...
    \ln K_{a}=-\frac{\Delta H}{R}\cdot\frac{1}{T}+\frac{\Delta S}{R}
\end{equation}

where ${\Delta G}$ is the Gibbs free enthalpy, ${R}$ is the ideal gas constant,
${T}$ is the temperature in Kelvins, ${K_{a}}$ is the equilibrium constant for
the transition, ${\Delta H}$ is the standard enthalpy of the transition,
${\Delta S}$ is the standard entropy. ${\theta}$ values correspond to signal
...
relations, from which values for ${\Delta H}$ and ${\Delta S}$ were derived by
...
${\alpha+\beta}$) upon heating in the range of
supramolecular transitions (${<}$ ca. \SI{45}{\celsius}). Increases in entropy, as
per Figure \ref{fig:thermo} thus suggest that the transitions may be a
consequence of hydrophobic interactions, driven by positive entropies of
(partial) dehydration.
% -------------------------- 
%START_TABLE
...
\begin{tabular}{ lccc }
  ...
  \hline
  Construct & ${T_t}$ (\si{\celsius}) & ${\Delta H}$ (\si{\kilo\cal\per\mole}) &
  ${\Delta S}$ (\si{\kilo\cal\per\mole}) \\
